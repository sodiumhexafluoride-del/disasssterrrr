...
import { initFirebase, isFirebaseActive, getFirebaseState, saveFirebaseState, listenToFirebaseState } from "./src/lib/firebaseServer";
...
async function startServer() {
  ...
  // If Firebase is active, try to fetch initial state and set up real-time listener in the background (non-blocking)
  if (isFirebaseActive()) {
    console.log("🔥 [Firebase] Background synchronization scheduled...");
    (async () => {
      try {
      ...
      console.log("🔥 [Firebase] Setting up real-time Firestore listener...");
      listenToFirebaseState((fbState) => {
        try {
          console.log("🔥 [Firebase] Real-time snapshot updated from Firestore. Writing to local cache.");
          fs.writeFileSync(DB_PATH, JSON.stringify(fbState, null, 2), "utf8");
        } catch (error) {
          console.error("🔥 [Firebase] Failed to write real-time update to db.json:", error);
        }
      });
    })();
  }
  ...
  app.get("/api/firebase/status", (req, res) => {
    res.json({
      active: isFirebaseActive(),
      projectId: process.env.FIREBASE_PROJECT_ID || null,
      usingAdc: !!process.env.GOOGLE_APPLICATION_CREDENTIALS,
      hasServiceAccount: !!(process.env.FIREBASE_PROJECT_ID && process.env.FIREBASE_CLIENT_EMAIL && process.env.FIREBASE_PRIVATE_KEY)
    });
  });
